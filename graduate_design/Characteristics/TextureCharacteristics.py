...
def glcm_feature(gray_gray_img,d_x, d_y,gray_level=16,feature="energy"):
...
def tamura_coarseness(gray_img, kmax):
...
# 输入为图像
def tamura_directionality(gray_img):
...
def tamura_linelikeness(gray_img, sita, dist):
...
#规整度
def tamura_regularity(gray_img, filter):
	pass
...
def dwt_feature(gray_img, wave_func = "haar"):
    ...
    feature = np.array()
    for i in range(4):
        np.append(feature, __dwt_average(dwt_metrix[i]))
        np.append(feature, __dwt_entropy(dwt_metrix[i]))
        np.append(feature, __dwt_sigma(dwt_metrix[i]))
        np.append(feature, __dwt_energy(dwt_metrix[i]))
    return feature
